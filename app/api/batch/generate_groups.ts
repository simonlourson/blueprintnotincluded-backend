    ...
    for (let oniItem of OniItem.oniItems) {
      ...
      if (spritesToGroup.length > 1) {
        ...
        for (let spriteModifier of oniItem.spriteGroup.spriteModifiers) {
          ...
          if (!ImageSource.isTextureLoaded(spriteInfo.imageId)) {
            let imageUrl = ImageSource.getUrl(spriteInfo.imageId);
            let brt = await PixiBackend.pixiBackend.getImage(imageUrl);
            ImageSource.setBaseTexture(spriteInfo.imageId, brt);
          }

          let texture = spriteInfo.getTexture();
          let sprite = PixiPolyfill.pixiPolyfill.getNewSprite(texture);
        ...
        let brt = PixiPolyfill.pixiPolyfill.getNewBaseRenderTexture({width: bounds.width, height: bounds.height});
        let rt = PixiPolyfill.pixiPolyfill.getNewRenderTexture(brt);

        PixiPolyfill.pixiPolyfill.pixiApp.renderer.render(container, rt);
