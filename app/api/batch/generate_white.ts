  ...
  async generateWhite(database: BExport) {
    ...
    for (let sourceTexture of sourceTextures)
    {
      ...
      if (!ImageSource.isTextureLoaded(sourceTexture)) {
        ...
        let brt = await PixiBackend.pixiBackend.getImageWhite(imageUrl);
        ImageSource.setBaseTexture(sourceTexture, brt);
      }

      let baseTexture = ImageSource.getBaseTexture(sourceTexture);
      
      let texture = PixiPolyfill.pixiPolyfill.getNewTextureWhole(baseTexture);
      
      let brt = PixiPolyfill.pixiPolyfill.getNewBaseRenderTexture({width: texture.width, height: texture.height});
      let rt = PixiPolyfill.pixiPolyfill.getNewRenderTexture(brt);

      let sprite = PixiPolyfill.pixiPolyfill.getNewSprite(texture);

      PixiPolyfill.pixiPolyfill.pixiApp.renderer.render(sprite, rt);
